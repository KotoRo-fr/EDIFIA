...
export default function ProgrammingPage() {
  ...
  const [programData, setProgramData] = useState<{program: any, footprint: any, sunAnalysis: any, budget: any}>({
    program: null, footprint: null, sunAnalysis: null, budget: null
  });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const fp = generateFootprint(parcelWidth, parcelDepth, cos, heightMax, {
        front: 3, side: 1.5, rear: 3,
      });
      const sun = analyzeSunExposure(prog);
      const bud = estimateBudget(prog.surfaces.total, project.project_type);
      setProgramData({ program: prog, footprint: fp, sunAnalysis: sun, budget: bud });
      setProgramGenerated(true);
    }, 50);
    return () => clearTimeout(timer);
  }, [project]);

  const { program, footprint, sunAnalysis, budget } = programData;
