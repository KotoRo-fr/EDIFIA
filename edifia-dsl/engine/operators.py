"""
Operateurs de comparaison pour le moteur de conformite EDIFIA.
"""
...
def between(value: float, min_val: float, max_val: float) -> bool:
    """Verifie que value est entre min_val et max_val (inclus)."""
    return float(min_val) <= float(value) <= float(max_val)


def in_set(value: Any, allowed: list) -> bool:
    """Verifie que value est dans la liste allowed."""
